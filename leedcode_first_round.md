...
>
>>1. `unordered_set` 和 `unordered_map` 的声明方式不同。
>>
>>> - `unordered_set` 的声明方式如下：
>>>
>>> ```C++
...
>>
>>2. `unordered_set` 和 `unordered_map` 的元素访问方式不同。
>>
>>> - `unordered_set` 的元素只能被访问，不能被修改。可以使用迭代器来遍历集合中的元素，例如：
>>>
>>> ```C++
...
>>
>>2. `unordered_set` 和 `unordered_map` 的查找方式不同。
>>
>>> - `unordered_set` 可以使用 `count()` 函数来判断一个元素是否在集合中出现过，例如：
>>>
>>> ```C++
>>> if (mySet.count(10)) {
...
>```
>
>> - `[col]`：这是lambda表达式的捕获列表，用于指定在lambda函数体中要使用的变量。在这种情况下，`col`是作为捕获的变量，表示当前要排序的列索引。
>>>
...
>`stack` C++ 中的栈（Stack）是一种数据结构，它遵循后进先出（Last-In-First-Out，LIFO）的原则。这意味着最后放入栈中的元素将会最先被取出。栈通常用于管理函数调用、表达式求值、内存分配等各种计算机程序中的操作。
>
>在 C++ 中，栈可以通过标准库提供的 `std::stack` 类来实现，也可以通过数组或链表自行实现。下面是关于 C++ 栈的一些重要信息和操作：
>
>> 1. **std::stack 类：** C++ 标准库提供了一个名为 `std::stack` 的模板类，用于实现栈数据结构。它位于 `<stack>` 头文件中，并基于其他容器（如 `std::deque` 或 `std::vector`）来实现。您可以使用 `push` 来将元素推入栈顶，使用 `pop` 来弹出栈顶元素，使用 `top` 来获取栈顶元素，以及使用 `empty` 来检查栈是否为空。
>>
>>    ```C++
>>    #include <iostream>
>>    #include <stack>
...
>>    ```
>>
>> 2. **手动实现栈：** 您也可以使用数组或链表手动实现栈。在这种情况下，您需要自己编写代码来执行推入、弹出和其他栈操作。通常，链表更适合实现栈，因为它们允许动态大小的栈。
>>
>> 3. **应用场景：** 栈在计算机科学和编程中有广泛的应用。一些常见的用途包括函数调用的调用栈、表达式求值（例如后缀表达式）、括号匹配检查、深度优先搜索算法中的状态跟踪等。
>>
>> 4. **时间复杂度：** 栈的基本操作（push、pop、top、empty）通常具有 O(1) 的时间复杂度，因为它们只涉及栈顶元素。然而，如果底层容器是动态数组（如 `std::vector`）的一部分，插入和删除操作可能会导致重新分配内存，这些操作可能会具有较高的时间复杂度。
>>
>> 总之，栈是一种常见的数据结构，用于管理数据的后进先出存储和访问。在 C++ 中，您可以使用标准库提供的 `std::stack` 类来方便地操作栈，也可以手动实现栈来满足特定需求。栈在编程中的应用非常广泛，是解决各种问题的有用工具。
...
> `优先队列` C++ 中的优先队列（Priority Queue）是一种特殊的队列数据结构，它根据元素的优先级来确定出队顺序。具体来说，优先队列中的元素是按照某种优先级顺序排列的，而不是按照它们进入队列的顺序。在优先队列中，拥有最高优先级的元素将首先被取出。
>
> C++ 中的优先队列通常使用堆（Heap）数据结构来实现，堆是一种二叉树结构，它保证了父节点的优先级高于其子节点。这使得在优先队列中插入和删除元素的时间复杂度保持在 O(log n) 的水平，其中 n 是队列中的元素数量。
>
> 以下是关于 C++ 中优先队列的一些重要信息和操作：
>
> > 1. **std::priority_queue 类：** C++ 标准库提供了一个名为 `std::priority_queue` 的模板类，用于实现优先队列。它位于 `<queue>` 头文件中，默认情况下，它使用的是大顶堆（即拥有最大值的元素在堆顶），但您可以通过提供自定义比较函数来创建小顶堆。您可以使用 `push` 来插入元素，使用 `pop` 来移除堆顶元素，使用 `top` 来获取堆顶元素，以及使用 `empty` 来检查队列是否为空。
> >
> >    ```C++
> >    #include <iostream>
> >    #include <queue>
...
> >    ```
> >
> > 2. **自定义比较函数：** 如果需要创建小顶堆，您可以提供一个自定义比较函数来改变元素之间的优先级比较规则
> >
> >    ```C++
> >    #include <iostream>
> >    #include <queue>
...
> >    ```
> >
> > 3. **应用场景：** 优先队列在许多算法和问题中都有重要应用，如最小堆和最大堆排序、Dijkstra 最短路径算法、Prim 最小生成树算法、任务调度、事件驱动模拟等。
> >
> > 4. **时间复杂度：** 优先队列的插入和删除操作通常具有 O(log n) 的时间复杂度，其中 n 是队列中的元素数量。获取队列顶部元素（最高优先级元素）的操作是 O(1)。
> >
> > 总之，C++ 中的优先队列是一种非常有用的数据结构，用于根据元素的优先级来确定出队顺序。它通过堆实现，提供了高效的插入和删除操作，适用于许多算法和问题，是解决各种优先级相关任务的有力工具。
...
>
>这里的 `prev` 指针是用来追踪前一个节点的。当 `prev` 不为空时，表示当前节点不是链表的头节点，需要更新前一个节点的 `next` 指针。如果 `prev` 为空，说明当前节点是头节点，不需要更新前一个节点的 `next` 指针。因此，在交换节点时，我们需要判断 `prev` 是否为空。如果 `prev` 不为空，则将其 `next` 指针指向交换后的新节点 `nextNode`；如果 `prev` 为空，说明当前节点是头节点，将新节点 `nextNode` 设置为新的头节点。这样做是为了确保交换后的链表能够正确连接，保持链表的完整性。
...
>`std::remove()` 是 C++ 标准库中的一个算法函数，用于从容器中删除指定值的所有元素。然而，与其名称可能暗示的不同，`std::remove` 不会真正地删除元素，而是将匹配的元素移到容器的末尾，并返回一个指向新逻辑结尾的迭代器。这允许您稍后通过调整容器的大小来真正删除这些元素。
>
>以下是 `std::remove` 函数的签名：
>
>```
>cppCopy codetemplate <class ForwardIt, class T>
>ForwardIt remove(ForwardIt first, ForwardIt last, const T& value);
>```
>
>- `ForwardIt` 是前向迭代器（forward iterator）的类型，通常是容器的迭代器类型。
>- `first` 是要处理的范围的起始位置的迭代器。
>- `last` 是要处理的范围的结束位置的迭代器（不包括这个位置）。
>- `value` 是要移除的元素的值。
>
>`std::remove` 返回一个迭代器，该迭代器指向新逻辑结尾的位置。新逻辑结尾之前的元素仍然是原始容器中的元素，但它们的值可能已经改变了。
>
>以下是一个示例，展示了如何使用 `std::remove` 来从容器中删除指定值的元素：
>
>```C++
>#include <iostream>
>#include <vector>
...
>
>    // 使用 std::remove 将值为 2 的元素移到容器的末尾
>    auto newEnd = std::remove(numbers.begin(), numbers.end(), 2);
>
>    // 使用 erase 方法删除新逻辑结尾之后的元素
...
>```
>
>注意，在上面的示例中，`std::remove` 将值为 2 的元素移到了容器的末尾，然后使用 `erase` 方法删除了新逻辑结尾之后的元素，从而实际删除了匹配的元素。
>
>需要注意的是，`std::remove` 不会改变容器的大小，它只是重新排列元素的顺序。如果您希望删除匹配的元素并缩小容器的大小，需要在调用 `std::remove` 后使用容器的 `erase` 方法。这种设计的好处是避免了不必要的元素复制和内存分配操作。

>`vector.erase()` 是 C++ 标准库中的一个函数，用于从 `std::vector` 容器中删除一个或多个元素。这个函数允许您指定要删除的元素的范围或位置，并将容器中的元素重新排列以填补删除的位置。以下是 `vector.erase()` 的两种常见用法：
>
>1. **删除单个元素：**
>
>```C++
...
>```
>
>在上面的示例中，`vector.erase()` 用于删除索引为2的元素（值为3），然后重新排列容器中的元素。
>
>1. **删除范围内的元素：**
>
>```C++
...
>```
>
>在上面的示例中，`vector.erase()` 用于删除索引为1到2的元素（值为2和3），并且重新排列容器中的元素。
>
>需要注意的是，`vector.erase()` 在删除元素后会改变容器的大小。如果您只想删除元素而不改变容器的大小，可以使用 `std::remove` 算法，然后再使用 `vector.erase()` 来删除新逻辑结尾之后的元素。这是一种常见的用法，用于删除多个匹配的元素，但仅在一次迭代中重新排列元素。
>
>总之，`vector.erase()` 是一个用于删除 `std::vector` 中元素的重要函数，它可以用于删除单个元素或指定范围内的元素，并且会改变容器的大小。删除元素后，容器中的元素将被重新排列以填补删除的位置。
...
`DFS`  `递归` 中序遍历时，判断当前节点是否大于中序遍历的前一个节点，如果大于，说明满足BST，继续遍历；否则直接返回false

### Code&&Analysis❤️

执行时间击败98.7%，消耗内存击败32.83%❤️❤️❤️

## 99.恢复二叉搜索树

### Thought

`DFS`  `递归` 中序遍历时，判断当前节点是否大于中序遍历的前一个节点，如果大于，创建`error_node_1`记录节点，继续遍历，直到第二次出现大于，创建`error_node_2`记录节点，等到遍历结束，题目要求不改变树的结构，因此交换两个错误节点的值

### Code&&Analysis❤️

执行时间击败66.62%，消耗内存击败81.99%❤️❤️❤️

## 100.相同的树

### Thought

`DFS`  `递归` 同时中序遍历二叉树`p`和`q`，创建`flag`记录两者是否相同，如果相同，就继续遍历，直到结束返回`flag = true`；否则返回`flag = false`

### Code&&Analysis💞

执行时间击败100%，消耗内存击败84.1%💞💞💞

## 101.对称二叉树

### Thought

`DFS`  `递归` 左右对称，也就是左右子树相同，同时遍历左右子树即可

### Code&&Analysis❤️

执行时间击败37.94%，消耗内存击败87.27%❤️❤️❤️

## 102.二叉树的层序遍历

### Thought

`BFS` `队列` 利用队列先进先出的特性可以实现层序遍历，但需要注意的最后结果是分层的，所以在处理的时候需要添加一个当前层数的`for`循环，执行完后再进行下一层的遍历

### Code&&Analysis💕

执行时间击败100%，消耗内存击败27.23%💕💕💕

## 103.二叉树的锯齿形遍历

### Thought

`BFS` `双端队列` 锯齿形遍历相当于每遍历一层就改变遍历方向的层序遍历，可以根据双端队列的特性，双数层时，头取从左到右尾插；单数层时，尾取从右到左头插，依然需要进行分层处理

### Code&&Analysis💕

执行时间击败100%，消耗内存击败50.88%💕💕💕

## 104.二叉树的最大深度

### Thought

`DFS` `递归` 如果我们知道了左右子树的最大深度`l`和`r`，那么二叉树的最大深度即`std::max(l, r) + 1`，然后左右子树的最大深度又可以通过同样的方式获得，将大问题不断缩写成小问题，这就是递归

### Code&&Analysis

执行时间击败70.71%，消耗内存击败36.48%

## 105.从前序与中序遍历序列构造二叉树

### Thought

`DFS`  `递归` 先序遍历的顺序是根左右，中序遍历的顺序是左根右，所以我们只需要在先序遍历中得到根节点，然后在中序遍历中找到根节点的位置，它的左边就是左子树的节点，右边就是右子树的节点，同理，左子树的根节点和左右子树，右子树的根节点和左右子树递归进行就好了
...
>
> ```C++
> if (it != inorder.end()) int index = std::distance(inorder.begin(), it);
> ```
...
执行时间击败55.84%，消耗内存击败24.29%

## 106.从中序与后序遍历构造二叉树

### Thought

`DFS`  `递归` 本题跟上一题神似，只不过变为了中序与后序遍历，中序遍历的顺序是左根右，后序遍历的顺序是左右根，同样可以通过后序遍历得到根节点，接下来的操作就乏善可陈了，参照上一题即可

### Code&&Analysis

执行时间击败60.79%，消耗内存击败42.9%

## 107.二叉树的层序遍历 II

### Thought

`BFS` `队列` 跟[二叉树的层序遍历](#102.二叉树的层序遍历) 没有什么区别，就是最后的时候翻转一下`res`数组就行了

#### new:

`链表` 这道题可以将`res`结构改成链表，以后每层遍历完就将结果放到链表的第一位，这样就无需翻转了

### Code&&Analysis

执行时间击败72.53%，消耗内存击败60.54%

## 108.将有序数组转换成二叉搜索树

### Thought

`递归` 因为`nums`升序，其实就相当于`BST`的中序遍历，我们可以选取任一个元素作为根节点，以节点的左边元素构建左子树，以节点的右边元素构建右子树，又因为题目要求高度平衡，所以选择`nums`的中间元素作为根节点

### Code&&Analysis

执行时间击败45.39%，消耗内存击败39.29%

## 109.有序链表转换二叉搜索树

### Thought

`链表` `递归` 本题跟上一题神似，只需要手动创建`nums`数组存储链表中的元素，并把结构改为链表即可

### Code&&Analysis❤️

执行时间击败99.63%，消耗内存击败12.31%❤️❤️❤️

## 110.平衡二叉树

### Thought

`递归` `DFS` 自底向上的递归，类似于后序遍历，对于当前遍历的节点，先递归判断其左右子树是否平衡，再判断以当前节点为根的子树是否平衡。如果一颗子树平衡，则返回其高度，否则返回`-1`。如果一颗子树不平衡，那么整个二叉树一定不平衡

### Code&&Analysis❤️

执行时间击败88.33%，消耗内存击败19.5%❤️❤️❤️

## 111.二叉树的最小深度

### Thought

`递归` 本题需要搞清的是叶子节点的定义：当左孩子和右孩子都为`nullptr`时叫做叶子结点
...
执行时间击败37.39%，消耗内存击败23.75%

## 112.路劲总和

### Thought

`递归` `DFS` 一直向下找到叶子节点，如果到叶子节点时`sum == 0`，说明找到了一条符合要求的路径

### Code&&Analysis

执行时间击败51.10%，消耗内存击败62.99%

## 113.路径总和 II

### Thought

`递归` 二叉树问题大多数都可以用递归和迭代的方式求解。我们必须使用一个变量 res 保存最终的所有路径结果，用一个变量 path 保存每条路径。另外需要记录路径和，我们反其道而行之，记录到达每个节点时的sum - 「路径和」；如果遍历到叶子节点的时候，sum - 「路径和」 恰好等于叶子节点的值，那么这条从根节点到叶子节点的路径即为一条满足题目的路径。
...
执行时间击败48.84%，消耗内存击败35.31%

## 114.二叉树展开为链表

### Thought

`递归` 可以发现展开的顺序其实就是二叉树的先序遍历。
...
- 状态定义：`dp[i][j]` 代表 `T` 前 `i` 字符串可以由 `S` `j` 字符串组成最多个数
...
执行时间击败77.76%，消耗内存击败38.97%

# End

  虽然还是留有一些题解的优化和少数题解的思路尚未完成，但是，第一阶段无专题按序刷题自此暂且告一段落了。从这篇md的第一个字的写下，迄今已三月有余，中途有过磕磕碰碰、有过修修改改，现在想想还是蛮有成就感。稍有遗憾的是，结尾结束的有些草率，毕竟随着第二阶段专题刷题的飞速进行，已经‘拖稿’很久了。还是希望之后的空闲时间能完善这一部分吧。

  路漫漫其修远兮，吾将上下而求索，今展筆撰跋於此，望与君共勉！
